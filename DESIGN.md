...
| Client testimony | `src/app/engine-network/testimonial.ts::NICK_TESTIMONIAL`, `src/app/engine-network/ClientVoice.tsx::ClientVoice`, `src/app/engine-network/FramedClientVoice.tsx::FramedClientVoice`, `src/app/engine-network/EngineNetworkPage.tsx::EngineNetworkPage` and `src/app/_design-lab/testimonial-reference/page.tsx::TestimonialReferencePage` | Shared approved wording, the production engraved-frame treatment and unrouted historical review references |
...
| Match set-piece | `src/app/engine-network/CurrencyMatch.tsx` | Cross-currency SVG diagram, its labelled-image semantics and the currency-swap choreography (clip window, beam gradient, two-leg route) |
...
- Gold as a state signal: the match set-piece reads unmatched-to-matched entirely inside the three gold tiers, introducing no fourth hue. Before the swap the route leg, the amount chip's rule and its figure are `--gold` on an `--obsidian-2` fill; after it the second leg, the chip rule, the figure and the destination's euro line are `--gold-bright` over a gold wash. The state is never colour-only — the figure itself changes currency and the MATCHED text plate stamps in.
...
- Match set-piece choreography: one 7.6s cycle with a deliberately short ~0.6s conversion beat at the FX node — the swap is a machine resolving a figure, so it snaps rather than dwells. Ten keyframe animations share that cycle and its 0.4s delay so the beam, the odometer roll, the plate cross-fade, the ping rings and the MATCHED stamp stay locked to the chip's position on the motion path. The amounts roll like a mechanical counter inside a `clipPath` window sized to the chip plate, swept by a gold gradient beam borrowed from the hero's scanning-gate language. The chip carries two stacked plates cross-faded on opacity rather than one recoloured plate, and the roll and sweep distances are custom properties (`--ecm-roll`, `--ecm-beam-travel`) because the chip geometry widens at the mobile breakpoint. Recorded exception: `ecmnet-arrive` animates `fill` on the destination's euro line, the one place this system animates paint rather than transform or opacity; it is a single short text node with no layout effect.
...
- Client testimony (`.eng-client-frame`): the homepage uses the selected ghost-aperture treatment with optically balanced outlined quotation marks, horizontal rules, the approved two-paragraph Nick Brookes wording and centred portrait attribution. The opening and closing glyphs use separate responsive translations because their visible ink is asymmetric inside the display font’s line boxes. The former `/testimonial/` page is retired from routing and preserved under `src/app/_design-lab/testimonial-reference/`, with the earlier portrait positions and exact single-paragraph `origin/main` reference available for future copy or design review.
...
- The page is one narrative in a fixed section order: hero → process → ledger ("the evidence, structured") → tracing ("the evidence, connected") → match ("the evidence, matched") → capabilities → outcome (section id "proof") → audiences and client testimony → engage → footer. Nick Brookes’s approved testimony follows the audience cards as named client proof, then hands directly into the engagement plate. The three set-pieces tell a single transaction's story — the arc runs pages → ledger → network → finding. Do not reorder sections or break the story's continuity.
...
- Known layout constraint (match set-piece): the three stations sit on one horizontal row inside a fixed `-20 0 600 200` viewBox, so below 768px every SVG user unit scales down hard and the station names are capped at 17px in user units — above that "INTL TRANSFER" and "DEUTSCHE ****9878" close to within a few units and read as one run. At 390px the names render at roughly 8.6px and the sub-labels at 6.8px, with the diagram claiming 8px of the plate's own padding on each side below 480px to recover scale. Raising phone legibility further needs a stacked mobile layout (stations on separate rows, taller viewBox), not another type increase. Approved, not yet implemented.
...
There is no automated visual test suite; evidence is the focused Node analytics Interface suite, lint, static build and headed real-GPU dev-browser checks. Analytics verification also inspects captured payloads for excluded data, confirms development makes no ingestion request and proves the built application has no rrweb or recorder implementation chunk. Keep commands and server lifecycle in `AGENTS.md`. Latest testimonial run: 2026-07-29, after selecting the ghost-aperture treatment and removing the review controls. 1440x900 and 390x900 passed: both marks remained inside the frame with optically balanced responsive translations, the two approved paragraphs and attribution remained visible, the footer contained no preview control, focus outlines remained visible and there were zero console/page errors or horizontal overflow. Latest match set-piece run: 2026-07-29, after the currency-swap choreography and type pass. 1440x900, 390x900 and 320x690 all passed: zero console/page errors, zero horizontal overflow, no station label overlapping its neighbour or clearing the plate's engraved inner frame, and the live loop verified unpaused (chip travels 0% to the 50% dwell to 100% and repeats, with the currency flipping inside the dwell). Latest hero-matrix run: 2026-07-20, after the mobile visibility pass. All seven viewports passed every gate: one canvas per normal load, intact forced fallback, same-canvas live resize, lifecycle pause and resume, zero console/page errors and zero horizontal overflow. The six viewports at or above 768px retained their existing copy, header action and unshifted canvas.
