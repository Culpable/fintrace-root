...
import { PUBLIC_ROUTES, blockExternalRequests, waitForStableDocument } from './agent.routes'
...
function gzipBytes(paths: string[]) {
  return paths.reduce((total, path) => total + gzipSync(readFileSync(resolve('out', path.slice(1)))).byteLength, 0)
}
