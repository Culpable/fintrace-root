...
export function assertBuiltRouteInventory() {
  for (const { route } of PUBLIC_ROUTES) {
    ...
    if (!existsSync(outputFile)) {
      throw new Error(`Built route is missing at ${outputFile}. Run npm run build first.`)
    }
  }
  if (!existsSync(resolve('out/404.html'))) {
    throw new Error('Built 404 document is missing. Run npm run build first.')
  }
}
