# FinTrace Site Pages Plan — Contact, About, Engagement & Pricing, 404

<critical_warning>
> **CRITICAL WARNING:** This plan executes ONLY AFTER two other plans have fully landed: `documents/todo/engine_network_audit_fix_plan.md` (the canonical-home audit fixes) and `documents/todo/fintrace_github_pages_deployment_plan.md` (Engine Network promoted to `/`, gallery moved to `/internal-design/`, production metadata, sitemap and GitHub Pages live at `https://fintrace.com.au/`). This plan's steps assume the post-deployment file layout — a shared `EngineNetworkPage.tsx`, a production `src/app/page.tsx` wrapper, `src/app/sitemap.ts` with one URL, and no `.eng-lab-chip` on the flagship. If those files do not exist in that shape, STOP and execute the prerequisite plans first.
</critical_warning>
...
<important_note>
> **IMPORTANT NOTE:** The contact form cannot go live without a Formspree form ID that only the user can create (Step 4 ships with a placeholder constant). This is the single blocking user action in the plan. Everything else — pages, copy, styling, navigation, sitemap — proceeds without user input.
</important_note>

## 1. Goal

`/engine-network/` (serving at `/` post-deployment) is the canonical FinTrace home page, but it is the ONLY page. A legal buyer who wants to check who is behind the service, how engagement and pricing work, or how to make contact has nowhere to go except a `mailto:` link — and a mistyped URL lands on an unbranded default 404. Build the four remaining production pages so the site works end to end for its buyers (family law firms, public trustees and government legal, forensic accountants, insolvency practitioners, estate/financial-abuse teams):
...
3. **`/contact/`** — a Formspree-backed enquiry form (modelled on the proven embeddings and bulma-root implementations) plus a structured "what happens next" panel and a direct mailto path.
...
| Header (flagship) | Wordmark (→ `/`) + section anchors Process/Ledger/Tracing/Proof + gold `eng-btn-sm` button "Request assessment" → `mailto:hello@fintrace.com.au`. Mobile (≤767 px) hides the text links, keeps wordmark + button (rule at `engine-network.css:1093`). |
| Hero CTAs | Gold "Request a matter assessment" → `mailto:hello@fintrace.com.au`; ghost "See how the engine works" → `#process`. |
| `#engage` CTA plate | Gold "Request a matter assessment" → mailto; ghost `hello@fintrace.com.au` → mailto. |
...
| Email | `hello@fintrace.com.au` appears throughout the copy; Google Workspace MX records exist on the domain, but the deployment plan notes the mailbox itself was never created/verified. |
...
The site converts through a single `mailto:` and answers none of the pre-engagement questions a legal buyer asks (who are you, how do you charge, how do we start, what do we receive). Government procurement in particular expects an engagement/pricing explanation before anyone emails. A default-styled 404 breaks the premium illusion the flagship builds. And a bare mailto leaks enquiries: it fails on machines without a configured mail client and gives no structure to what the enquirer should include.
...
- **REQ-7 (MUST):** Home-page changes are limited to exactly four edits: (a) footer page-nav block, (b) hero gold CTA `href` mailto → `/contact/`, (c) header gold button `href` mailto → `/contact/`, (d) `#engage` gold CTA `href` mailto → `/contact/`. The `#engage` ghost button stays `mailto:hello@fintrace.com.au` (a deliberate direct-email escape hatch). No other home copy, layout, animation or style changes.
...
- **Form fallback order:** Formspree fetch → on any failure, the error panel's `mailto:hello@fintrace.com.au` link with typed values preserved on screen for manual copying.
- **Formspree ID fallback:** until the real ID replaces `REPLACE_WITH_FINTRACE_FORM_ID`, every submission takes the error path with the mailto fallback — the page degrades to a styled mailto contact page rather than breaking.
...
**Functional:**
- [ ] `/about/`, `/engagement/`, `/contact/` render with shared header/footer chrome at both viewports
- [ ] Contact form: valid submission → success state; forced failure (placeholder/blocked ID) → error panel with mailto and preserved values; honeypot field invisible and excluded from tab order
- [ ] 404: any unknown route (e.g. `/nope/`) serves the branded page; button returns to `/`
- [ ] Header button targets: `/contact/` everywhere except on `/contact/` where it is `#enquire`

**Copy/claims:**
- [ ] Every rendered string equals its copy-deck entry (`textContent` assertions)
- [ ] Grep of new page sources finds no straight apostrophes in UI strings, no ` , and ` three-item lists, no claim outside the REQ-3 set

**SEO:**
- [ ] `out/sitemap.xml` has exactly 4 `<loc>` entries; `out/about/index.html`, `out/engagement/index.html`, `out/contact/index.html` each carry a self canonical and no `noindex`
- [ ] `out/404.html` exists and contains the branded markup
- [ ] No public page contains an `/internal-design/` href

**Compatibility:**
- [ ] `git diff --stat` touches only `src/app/` production files named in this plan and `documents/`
- [ ] Candidate lab routes byte-identical (`git diff` empty for `src/app/{ledger,trace,clarity,engine,exhibit,chambers,engine-refined,engine-trace,engine-ledger,engine-flow}/`)
- [ ] No un-prefixed keyframes (grep)

**Ops/Docs:**
- [ ] Lint zero errors; build exports; hero matrix screenshots saved with prefix `r7-`
- [ ] `AGENTS.md`, design plan and README updated per Step 7
...
- **Why CTA hrefs retarget to `/contact/` despite the hero-matrix cost:** a form converts better than a bare mailto (works without a configured mail client, structures the enquiry) and a split experience — some buttons mailto, some form — would be incoherent. The ghost `hello@fintrace.com.au` button in `#engage` deliberately stays mailto as the direct-email path. The hero change is `href`-only, so the mandatory matrix re-run is cheap.
...
- **Formspree account context:** the user's existing Formspree account (the one holding `xrbgdgwq` and `xojvwybl`; account email believed to be `hello@webpop.com.au`) is where the new form should be created. Notification recipient should be set to `hello@fintrace.com.au` once that mailbox exists — the deployment plan notes it was never created; until then, notifications go to the account's default email. Submissions are always readable in the Formspree dashboard regardless.
...
### Step 1: Shared chrome — SiteHeader, SiteFooter, page-hero styles, home integration
...
1. **`SiteChrome.tsx`** (Server Component, colocated with the system that styles it) exports:
   - `SiteHeader({ contactHref = '/contact/' })`: the existing header markup with wordmark → `/` and nav links `About` → `/about/`, `Engagement` → `/engagement/`, `Contact` → `/contact/`, plus the gold `eng-btn-sm` button `Request assessment` → `contactHref`. Reuses `.eng-header`/`.eng-header-nav` classes untouched; the ≤767 px collapse rule applies as-is. The HOME page does NOT use `SiteHeader` — its audit-locked header (section anchors) stays inline in the shared page component; only its gold button `href` changes (`mailto:hello@fintrace.com.au` → `/contact/`).
...
3. **Hero CTA retarget** in `Hero.tsx`: gold button `href` `mailto:hello@fintrace.com.au` → `/contact/`. Attribute-only; no text/geometry/style change. (Triggers the seven-viewport matrix — see Testing Plan.)
4. **`#engage` CTA retarget** in the shared page component: gold button `href` → `/contact/`; ghost button unchanged (mailto).
...
- `Hero.tsx` contains zero `mailto:` strings; the shared page's `#engage` block contains exactly one `mailto:hello@fintrace.com.au` (the ghost button) (grep).
...
### Step 2: `/about/` — the service story
...
| S1 body ¶2 | `Those findings closely matched the analysis the instructing lawyer prepared independently — the same anomalies, the same traced transfers, at a fifth of the time. That matter set the standard every engagement since is held to.` |
...
| S3 body ¶1 | `Every line in the ledger and every finding in the report cites the exact page of the source PDF it came from. Anything the engine asserts, a human can verify in seconds — which is what lets a finding stand in negotiation, before a tribunal or in court.` |
...
### Step 3: `/engagement/` — engagement & pricing
...
| CTA gold button | `Request a matter assessment` → `/contact/` |
| CTA ghost button | `hello@fintrace.com.au` → `mailto:hello@fintrace.com.au` |
...
### Step 4: `/contact/` — enquiry form on Formspree

**Objective:** Convert enquiries through a structured, on-brand form with a bulletproof direct-email fallback, modelled on the proven bulma-root/embeddings implementations.
...
| Direct link | `hello@fintrace.com.au` → `mailto:hello@fintrace.com.au` |
...
| Success panel | `Enquiry received. We’ll reply from hello@fintrace.com.au.` |
| Error panel heading | `Enquiry not sent` |
| Error panel body | `Your details are still filled in above — you can retry, or email hello@fintrace.com.au and we’ll respond directly.` |
...
- Module-scope constants with a comment naming the swap ceremony: `FORMSPREE_FORM_ID = 'REPLACE_WITH_FINTRACE_FORM_ID'`, and `FORMSPREE_ACTION` built from it via a template literal as `https://formspree.io/f/<FORMSPREE_FORM_ID>` (mirrors bulma-root's two-constant pattern).
...
**User action (the only one):** create a new Formspree form in the existing account (the one holding `xrbgdgwq`/`xojvwybl`), name it `FinTrace — matter enquiries`, set the notification recipient (see §4.2 mailbox caveat), and supply the form ID to replace `REPLACE_WITH_FINTRACE_FORM_ID`. Until then the form degrades gracefully to the error-panel mailto path.
...
- With the placeholder ID: submitting a filled form shows the error panel (`role="alert"`, heading `Enquiry not sent`) and every typed value survives in the fields (assert `input.value` post-failure).
- With a real/test ID (once supplied): a submission returns HTTP 200 from Formspree and shows the success panel (`role="status"`); the submission appears in the Formspree dashboard.
...
### Step 6: SEO — sitemap amendment and metadata verification
...
### Step 7: Documentation synchronisation
...
| `documents/todo/fintrace_github_pages_deployment_plan.md` | The superseded sitemap requirement (REQ-28) and internal-route policy | Sitemap becomes four URLs; internal routes stay unlinked/noindex; everything else in that plan remains true |
...
| Scope guard | `git diff --stat` | Only: `src/app/{about,engagement,contact}/`, `src/app/not-found.tsx`, `src/app/not-found.css`, `src/app/sitemap.ts`, `src/app/engine-network/{SiteChrome.tsx,site-pages.css,engine-network.css,Hero.tsx,EngineNetworkPage.tsx}`, `AGENTS.md`, `README.md`, `documents/*` |
...
2. **Contact form state machine (1440x900 and 390x900)**
   - Action: submit empty (native `required` blocks); fill all fields, submit with the placeholder ID (error path); if the real ID is available, submit once with clearly-marked test content (`TEST — please ignore` in the message) and confirm receipt in the Formspree dashboard; keyboard-only pass through all fields.
...
3. **Home integration (1440x900 and 390x900)**
   - Action: full-page sweep of `/`; click-test header button, hero gold CTA, `#engage` gold CTA (→ `/contact/`) and the ghost mailto; inspect footer nav.
