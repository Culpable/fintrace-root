    ...
    const issues = await page.locator('form').evaluateAll(
      (forms, autosubmitAllowlist): WebMcpIssue[] => {
        ...
        forms.forEach((form, formIndex) => {
          ...
          // Collect without the `querySelectorAll` generic: the Workers runtime
          // types in scope declare their own global `Element`, which does not
          // satisfy the DOM constraint.
          const fields = [
