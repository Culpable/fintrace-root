...
/** Notify the caller once the first frame has rendered so the canvas can fade in. */
export type SceneReadyCallback = () => void
 ...
 * ------------------------------------------------------------------------- */
/**
 * Build the scene inside `mount` and return its disposer. The caller owns the
 * mount element and calls the returned function to tear everything down.
 */
export function mountEvidenceScene(mount: HTMLElement, onReady?: SceneReadyCallback): () => void {
    /* ------------------------------ Renderer ----------------------------- */
    ...
    try {
      ...
      // Nothing was created, so the disposer has nothing to undo.
      return () => {}
    }
